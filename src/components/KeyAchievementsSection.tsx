...
const KeyAchievementsSection = () => {
  const items = [
    "~25% improvement in workflow throughput across dealership operations",
    "30% reduction in staff turnover through structured coaching programmes",
    "100% stock accuracy with zero stockout incidents for spare parts",
    "100% SLA compliance across omnichannel payment operations",
  ];
      ...
      <div className="container mx-auto px-4">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-base text-muted-foreground max-w-xl mx-auto">Measurable outcomes from operations, people leadership, and process improvement.</p>
          </div>
            ...
            {items.map((text) => (
              <Card key={text} className="rounded-lg border border-border/70 bg-card shadow-sm hover:shadow-lg hover:-translate-y-1 hover:border-[hsl(var(--primary))]/25 transition-all duration-200">
                <CardContent className="p-6 flex flex-col items-center text-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-[hsl(var(--primary))]/15 flex items-center justify-center"><TrendingUp className="w-5 h-5 text-[hsl(var(--primary))]" /></div>
